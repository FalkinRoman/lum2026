...
@if ($variant === 'mobile')
    ...
    <article @class(['relative flex h-[440px] w-full flex-col overflow-hidden px-[28px] pt-[28px] pb-[28px] shadow-[3px_3px_0_0_rgba(0,0,0,0.25)]', $palette['bg']])>
        <div @class(['relative z-[1] flex items-center gap-[9px] text-[12px] font-medium uppercase leading-[12px] tracking-[0.6px]', $palette['tags']])>
...
@elseif ($variant === 'tablet')
    <article @class(['relative h-[320px] w-full overflow-hidden shadow-[3px_3px_0_0_rgba(0,0,0,0.25)]', $palette['bg']])>
        ...
        <div @class(['absolute bottom-[28px] left-[470px] flex items-center gap-[9px] text-[12px] font-medium uppercase leading-[12px] tracking-[0.6px]', $palette['tags']])>
            <span>{{ $post['tags'][0] }}</span>
            <span @class(['size-[3px] rounded-full', $palette['dot']])></span>
            <span>{{ $post['tags'][1] }}</span>
        </div>
...
@else
    <article @class(['relative h-[360px] w-full overflow-hidden shadow-[3px_3px_0_0_rgba(0,0,0,0.25)]', $palette['bg']])>
        ...
        <div @class(['absolute bottom-[28px] left-[396px] flex items-center gap-[9px] text-[12px] font-medium uppercase leading-[12px] tracking-[0.6px]', $palette['tags']])>
            <span>{{ $post['tags'][0] }}</span>
            <span @class(['size-[3px] rounded-full', $palette['dot']])></span>
            <span>{{ $post['tags'][1] }}</span>
        </div>
