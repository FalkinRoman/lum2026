...
>
    ...
    <div class="relative hidden h-full tab:block desk:hidden" data-lum-interior-panel>
        ...
        @endif
        <div @class(['absolute left-1/2 -translate-x-1/2 text-center text-lum-espresso', 'top-[145px]' => $showLogomark, 'top-[80px]' => ! $showLogomark]) data-lum-scroll-reveal>
        ...
        <div @class(['absolute left-1/2 h-[71px] w-[281px] -translate-x-1/2 -rotate-[1.4deg] bg-lum-orange opacity-56 shadow-[3px_3px_0_rgba(0,0,0,0.25)]', 'top-[217px]' => $showLogomark, 'top-[151px]' => ! $showLogomark])></div>
        ...
        <img
            ...
            alt=""
            data-lum-interior-single
            @class(['absolute left-0 h-[641px] w-full object-cover shadow-[3px_3px_0_rgba(0,0,0,0.25)]', 'top-[421px]' => $showLogomark, 'top-[356px]' => ! $showLogomark])
            width="960"
            height="641"
        >
        ...
        <div @class(['absolute left-1/2 flex w-[378px] -translate-x-1/2 flex-col items-center gap-[24px]', 'top-[1134px]' => $showLogomark, 'top-[1069px]' => ! $showLogomark])>
    ...
    <div class="relative hidden h-full desk:block" data-lum-interior-panel>
        ...
        @endif
        <div @class(['absolute left-1/2 -translate-x-1/2 text-center text-lum-espresso', 'top-[208px]' => $showLogomark, 'top-[160px]' => ! $showLogomark]) data-lum-scroll-reveal>
        ...
        <div @class(['absolute left-1/2 h-[80px] w-[440px] -translate-x-1/2 -rotate-[1.4deg] bg-lum-orange opacity-56 shadow-[3px_3px_0_rgba(0,0,0,0.25)]', 'top-[328.64px]' => $showLogomark, 'top-[292px]' => ! $showLogomark])></div>
        ...
        <div
            @class(['absolute left-0 h-[620px] w-[928px] overflow-hidden shadow-[3px_3px_0_rgba(0,0,0,0.25)]', 'top-[605px]' => $showLogomark, 'top-[556px]' => ! $showLogomark])
        ...
        <div
            @class(['absolute left-[992px] h-[620px] w-[928px] overflow-hidden shadow-[3px_3px_0_rgba(0,0,0,0.25)]', 'top-[604px]' => $showLogomark, 'top-[556px]' => ! $showLogomark])
        ...
        <button type="button" data-lum-interior-prev @class(['absolute flex size-[64px] rotate-90 items-center justify-center rounded-[50px] bg-lum-ivory p-[16px]', 'left-[72px] top-[884px]' => $showLogomark, 'left-[136px] top-[836px]' => ! $showLogomark]) aria-label="{{ __('lum.aria.previous') }}">
            <img src="{{ $img('interior/arrow.svg') }}" alt="" class="size-[32px]" width="32" height="32">
        </button>
        <button type="button" data-lum-interior-next @class(['absolute flex size-[64px] -scale-y-100 rotate-90 items-center justify-center rounded-[50px] bg-lum-ivory p-[16px]', 'left-[1784px] top-[884px]' => $showLogomark, 'left-[1784px] top-[836px]' => ! $showLogomark]) aria-label="{{ __('lum.aria.next') }}">
            <img src="{{ $img('interior/arrow.svg') }}" alt="" class="size-[32px]" width="32" height="32">
        </button>
        <div @class(['absolute left-[771px] flex w-[378px] flex-col items-center gap-[24px]', 'top-[1300px]' => $showLogomark, 'top-[1252px]' => ! $showLogomark])>
            <div class="flex w-full items-center justify-center gap-[14px]" data-lum-interior-progress data-line-class="h-px w-[42px] bg-lum-espresso opacity-40">
                @for ($i = 0; $i < $total; $i++)
                    <span class="inline-flex h-[19px] w-[42px] items-center justify-center" data-lum-interior-progress-item data-index="{{ $i }}">
                        <span data-lum-interior-progress-line @class(['h-px w-[42px] bg-lum-espresso opacity-40', 'hidden' => $i === $startIndex])></span>
