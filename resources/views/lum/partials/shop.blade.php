    ...
    <div class="absolute inset-0 z-10 flex items-center justify-center tab:hidden">
        <div class="flex w-[335px] flex-col items-center gap-[30px] text-center" data-lum-scroll-reveal>
            ...
            <div class="flex flex-col items-center gap-[24px]">
                <p class="lum-text-3 font-medium uppercase text-lum-ivory">{{ $shopTeaser['eyebrow'] ?? __('lum.shop.eyebrow') }}</p>
                <div class="pt-[12px] pb-[40px] text-lum-ivory">
                    <p class="font-serif text-[56px] leading-[56px]"><span class="font-normal not-italic">{{ $shopTeaser['title_normal'] ?? __('lum.shop.title_normal') }}</span></p>
                    <p class="font-serif text-[56px] font-medium italic leading-[56px]">{{ $shopTeaser['title_italic'] ?? __('lum.shop.title_italic') }}</p>
                </div>
                <a href="{{ route('shop') }}" class="lum-btn-ivory px-[24px] pt-[5px] pb-[4px] text-[14px] leading-[23px] tracking-[2.84px]">{{ $shopTeaser['cta'] ?? __('lum.shop.cta') }}</a>
            </div>
        </div>
    </div>
    ...
    <div class="absolute inset-0 z-10 hidden items-center justify-center tab:flex desk:hidden">
        <div class="flex w-[920px] flex-col items-center gap-[32px] text-center" data-lum-scroll-reveal>
            <div class="flex flex-col items-center gap-[24px]">
                ...
                <p class="lum-text-2 font-medium uppercase text-lum-ivory">{{ $shopTeaser['eyebrow'] ?? __('lum.shop.eyebrow') }}</p>
            </div>
            <div class="flex w-full items-center justify-center gap-[32px]">
                ...
                <h2 class="whitespace-nowrap font-serif text-[80px] leading-[80px] text-lum-ivory"><span class="font-normal not-italic">{{ $shopTeaser['title_normal'] ?? __('lum.shop.title_normal') }} </span><span class="italic">{{ $shopTeaser['title_italic'] ?? __('lum.shop.title_italic') }}</span></h2>
                <img src="{{ $img('shop/deco-right.svg') }}" alt="" class="w-[72px]" width="72" height="2">
            </div>
            <a href="{{ route('shop') }}" class="rounded-[50px] bg-lum-ivory px-[34px] pt-[6px] pb-[5px] text-[16px] font-extrabold uppercase leading-[25px] tracking-[3.2px] text-lum-espresso">{{ $shopTeaser['cta'] ?? __('lum.shop.cta') }}</a>
        </div>
    </div>
    ...
    <div class="absolute bottom-0 left-[80px] top-1/2 z-10 hidden w-[1760px] -translate-y-1/2 flex-col items-center gap-[44px] text-center desk:flex">
        <div class="flex w-full flex-col items-center gap-[44px]" data-lum-scroll-reveal>
            ...
            <div class="flex w-full flex-col items-center gap-[38px]">
                <p class="lum-eyebrow text-lum-ivory">{{ $shopTeaser['eyebrow'] ?? __('lum.shop.eyebrow') }}</p>
                <div class="flex w-full items-center justify-center gap-[32px]">
                    <img src="{{ $img('shop/deco-left.svg') }}" alt="" class="w-[108px] rotate-180 scale-y-[-1]" width="108" height="2">
                    <h2 class="lum-heading-1-hero text-lum-ivory"><span class="font-normal not-italic">{{ $shopTeaser['title_normal'] ?? __('lum.shop.title_normal') }} </span><span class="italic">{{ $shopTeaser['title_italic'] ?? __('lum.shop.title_italic') }}</span></h2>
                    <img src="{{ $img('shop/deco-right.svg') }}" alt="" class="w-[108px]" width="108" height="2">
                </div>
                <a href="{{ route('shop') }}" class="lum-btn-ivory">{{ $shopTeaser['cta'] ?? __('lum.shop.cta') }}</a>
            </div>
        </div>
    </div>
</section>
