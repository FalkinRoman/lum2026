<article @class(['relative h-[650px] w-[450px] border border-dashed border-lum-espresso bg-lum-sand', 'mt-[40px]' => ! empty($stacked)])>
    ...
    @if ($card['activeImgRotate'])
        <div class="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center">
            <div class="-rotate-[15deg]">
                <img src="{{ \App\Support\Content::mediaUrl($card['activeImg'] ?? null) }}" alt="" class="{{ $layout['tabImg'] }} {{ $card['activeImgClass'] }}">
            </div>
        </div>
    @else
        <img src="{{ \App\Support\Content::mediaUrl($card['activeImg'] ?? null) }}" alt="" class="absolute left-1/2 top-1/2 {{ $layout['tabImg'] }} -translate-x-1/2 -translate-y-1/2 {{ $card['activeImgClass'] }}">
    @endif
    ...
    <a href="{{ $card['href'] ?? \App\Support\Content::link($card['route'] ?? null) }}" class="lum-btn lum-btn-info absolute left-1/2 top-[574px] -translate-x-1/2 px-[24px] pt-[5px] pb-[4px] text-[14px] leading-[23px] tracking-[2.84px]">{{ __('lum.location.more_info') }}</a>
</article>
